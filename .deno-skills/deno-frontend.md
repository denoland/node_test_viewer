...
Fresh 2.x provides two main approaches for fetching data on the server. Choose
based on your needs:

### Approach A: Async Server Components (Simplest)

Use this when you just need to fetch and display data. This is the **recommended
default**.
...
Use this when you need to check authentication, perform redirects, or run logic
before rendering.
...
Tailwind CSS is optional---you don't need it to build a great Fresh app.
However, many developers prefer it for rapid styling. Fresh 2.x uses Vite for
builds, so Tailwind integrates via the Vite plugin.
